...
Defines the contract for `:generator` → `:models` emission: how the lexicon
JSON corpus is parsed, named, verified, and converted into idiomatic
Kotlin via KotlinPoet. Covers data-class shape, optionality semantics,
contextual splits, open-union encoding, naming rules, KDoc/deprecation
propagation, raw-bytes procedure surfaces, per-call proxy overrides, and
the per-NSID coverage manifest. Consumers of the generated SDK rely on
these requirements for source and binary stability across releases.
